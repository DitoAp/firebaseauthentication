...
import com.example.apkdatateman.databinding.ActivityMyListDataBinding
...
class MyListData : AppCompatActivity() {
    ...
    private var apkDataTeman = ArrayList<apk_data_teman>()
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    if (dataSnapshot.exists()) {
                        for (snapshot in dataSnapshot.children) {
                            val teman = snapshot.getValue(apk_data_teman::class.java)
                            teman?.key = snapshot.key
                            apkDataTeman.add(teman!!)
                        }
                        adapter = RecyclerViewAdapter(apkDataTeman, this@MyListData)
